...
数学の関数を説明すると、
```ruby
f(x,y) = x^2 + y^2 # ここで^はべき乗の意味
```
という関数`f`が定義されているとするとします。

`xに2、yに3`を与えると、`f`は`13`という値を返します。

Rubyのメソッドも同様に、与えられたデータに対して、あらかじめ定義された処理を行い、オブジェクトに影響を与えたり、結果としてオブジェクト（値など）を返します。
